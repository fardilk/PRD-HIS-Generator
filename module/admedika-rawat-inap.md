...
| No | Screenshot | Deskripsi |
|---|-----------|-----------|
| 001 | dari-proses-klaim-klik-upload-dokumen.png | **Navigation: Dashboard AdMedika Tabs** menampilkan 3 tabs navigation: **Tab 1: "Proses Klaim"** (sudah selesai), **Tab 2: "Upload Dokumen"** ← User klik tab ini (active/clickable hanya setelah proses klaim selesai), **Tab 3: "Daily Monitoring"**. Alternatif: dari Screenshot 005 (Calculation Result), user dapat klik button **[Next: Upload Dokumen]** untuk direct link ke tab. Sistem automatic navigate ke Tab "Upload Dokumen" dan load full interface. |
| 002 | selanjutnya-tampil-tab-upload-dokumen-klik-button-upload.png | **Tab Upload Dokumen Interface dengan Checklist:** **Header Section:** Title "Upload Dokumen Support Klaim" + Subtitle "Upload dokumen yang diperlukan untuk mempercepat proses approval klaim" + Klaim Reference Number (read-only): CLM-2026-001234. **Info Messages:** "Dokumen yang ditandai * adalah WAJIB diunggah" | "Format file: PDF, JPG, PNG \| Max: 10MB/file" | "Setelah semua dokumen wajib diunggah, Anda dapat submit klaim ke AdMedika". **Checklist - Grouped by Importance:** **MANDATORY** (3 items): ☐ *Medical Record/Ringkasan Klinis (PDF) ⭕ Not Uploaded, ☐ *Invoice/Rincian Biaya (PDF) ⭕ Not Uploaded, ☐ *Receipt/Bukti Pembayaran (PDF) ⭕ Not Uploaded. **CONDITIONAL** (3 items if applicable): ☐ Lab Results/Hasil Lab (PDF), ☐ Imaging Results/Hasil Radiologi (PDF), ☐ Medication Receipt/Bukti Obat (PDF). **OPTIONAL** (1 item): ☐ Other Supporting Documents (PDF/JPG/PNG). **Upload Interface per document:** Drag-drop area OR [Browse] button OR click upload area. **Status Indicators:** ⭕ Not Uploaded (gray) | ⏳ Uploading X% (progress) | ✅ Uploaded (green) | ❌ Error message (red). **Bottom Section - Completion Status & Actions:** Progress display "X of 6 mandatory documents uploaded" dengan checkmarks. **Action Buttons:** **[Submit Klaim dengan Dokumen]** (DISABLED if not all mandatory uploaded, tooltip explaining), **[Submit Klaim dengan Dokumen]** (ENABLED if all mandatory uploaded, tooltip "Submit klaim beserta dokumen ke AdMedika"), **[Save as Draft]**, **[Cancel]**. Clear mandatory vs optional indication with submit button control logic. |
...
| No | Screenshot | Deskripsi |
|---|-----------|-----------|
| 001 | dari-proses-klaim-ke-daily-monitoring.png | **Navigation: Dashboard AdMedika Tabs** menampilkan 3 tabs navigation: **Tab 1: "Proses Klaim"**, **Tab 2: "Upload Dokumen"**, **Tab 3: "Daily Monitoring"** ← User klik tab ini untuk access daily monitoring interface. Alternatif: dari dashboard sidebar, user dapat klik link "Daily Monitoring". Sistem automatic navigate ke Tab "Daily Monitoring" dan load full dashboard interface dengan list active patients. |
| 002 | tampil-tab-daily-monitoring-terdapat-data-disana.png | **Daily Monitoring Dashboard - Patient Monitoring Table:** **Header Section:** Title "Daily Monitoring - Benefit Usage Rawat Inap" + Subtitle "Monitor penggunaan benefit pasien AdMedika harian dan identifikasi risk cases". **Filter & Control:** Date Range picker, Risk Level dropdown (All/Normal/Caution/Alert/Excess), Status dropdown (All/Active/About to Discharge), Search Box (patient name/MR), [Manual Refresh] button, Auto-refresh info "Dashboard auto-refresh setiap 5 menit". **Table Columns:** [checkbox], No., Patient Name (clickable), MR Number, Room/Kamar, Admission Date (DD-MM-YYYY), Days Admitted, Benefit Available (Rp), Benefit Used (Rp), Benefit Remaining (Rp), Usage % (visual progress bar color-coded: Green 0-50%, Yellow 51-84%, Orange 85-99% with ⚠️, Red >100% with 🚨), Risk Indicator (no icon/⚠️ CAUTION/🚨 ALERT), Action Buttons ([View Detail]/[Add Data]). **Example data:** Pasien A (70%, ⚠️), Pasien B (84%, -), Pasien C (102%, 🚨). **Performance:** Load 100+ patients < 5 detik. |
| 003a | tampil-form-tambah-data-daily-monitoring.png | **Quick Add Monitoring Data Form:** Modal dialog "Tambah Data Monitoring - Input Cepat" terbuka saat user klik **[Add Data]** button. **Quick Form Fields:** ☐ **Select Patient** (required, dropdown with search/autocomplete, shows name + MR), ☐ **Service Category** (required, dropdown: Kamar/Obat/Tindakan/Pemeriksaan/Lain-lain), ☐ **Amount** (required, numeric Rp format, validation > 0), ☐ **Date of Service** (required, date picker default today, validation <= today), ☐ **Notes** (optional, textarea max 250 chars, placeholder "Tambahkan catatan (optional)"). **Action Buttons:** **[Save]** (submit quick entry), **[Cancel]** (close & discard), **[Expand Form]** link (go to detailed form 003b). **Validation on Submit:** All required fields harus filled, inline error messages jika validation fail dengan field highlighting. Simple, quick entry untuk rapid data input. |
| 003b | tampilan-form-tambah-daily-monitoring-complete.png | **Detailed Monitoring Data Form:** Modal dialog "Tambah Data Monitoring - Form Lengkap" menampilkan expanded form dengan multiple sections. **Section 1 - Patient Info (read-only):** Nama Pasien, Nomor Rekam Medis, Kamar, Tanggal Admission, Dokter Pengasuh. **Section 2 - Service Details:** ☐ Service Code (required, dropdown dengan search dari service master, auto-fill description & category), ☐ Service Description (auto-fill dari code, read-only), ☐ Service Category (auto-fill dari code, read-only), ☐ Service Date (required, date picker), ☐ Quantity (required, numeric default 1), ☐ Unit Price (read-only dari service master, Rp format), ☐ Total Amount (auto-calculated read-only: Qty × Unit Price). **Section 3 - Benefit Calculation (auto-calculated, real-time):** ☐ Mapped to Benefit Category (auto-suggest based on service code, user dapat change), ☐ Coverage % (read-only dari benefit rules), ☐ Estimated Benefit Amount (auto-calculated real-time: Total Amount × Coverage %), ☐ Co-payment Amount (auto-calculated: Total - Benefit OR fixed per rules). **Section 4 - Notes:** Textarea optional max 500 chars. **Real-time Calc Display:** Shows Service Amount | Coverage % | Benefit Amount | Co-payment instantly updating. **Action Buttons:** **[Save]** (submit detailed entry), **[Cancel]** (close), **[Collapse to Quick Form]** link. Full detail entry untuk precision input dengan auto-calculation. |
| 004 | daily-monitoring-berhasil-ditambahkan.png | **Confirmation Screen - Data Monitoring Disimpan:** Green success message card ✓ **"Data Berhasil Disimpan"** + subtitle "Data monitoring telah tersimpan di sistem" + timestamp "Disimpan pada: 12-Jan-2026 14:35:22". **Summary Data Entered:** Service Details (Service name, Category, Amount, Quantity, Date) + Benefit Calculation (Coverage %, Benefit Amount, Co-payment). **Updated Patient Status:** Patient: [Name] (MR-001). **Before Entry:** Benefit Used Rp 3.5M, Benefit Remaining Rp 1.5M, Usage % 70% (Green). **New Service Entry:** +Rp 1.2M (benefit amount). **After Entry:** Benefit Used Rp 4.7M, Benefit Remaining Rp 0.3M, Usage % 94% (Orange with ⚠️ CAUTION). **Visual Status Change:** arrow/highlight menunjukkan status update dari Green → Orange jika threshold crossed. **Alert Notification (if triggered):** "⚠️ Alert triggered untuk pasien ini! Usage sekarang 94%" (jika >85%, alert via Email/SMS/In-app). **Action Buttons:** **[Kembali ke Dashboard]** (return to 002, auto-refresh with new data), **[Tambah Data Lagi]** (open form again), **[Lihat Detail Pasien]** (view full patient detail dengan all monitoring entries). Success feedback + impact visualization. |
