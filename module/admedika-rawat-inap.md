...
| No | Screenshot | Deskripsi |
|---|-----------|-----------|
| 001 | dari-proses-klaim-ke-daily-monitoring.png | **Navigation: Dashboard AdMedika Tabs** menampilkan tab navigation: **Tab "Proses Klaim"**, **Tab "Upload Dokumen"**, **Tab "Daily Monitoring"** ← User klik tab ini untuk access daily monitoring interface. Alternatif: dari dashboard sidebar, user dapat klik link "Daily Monitoring". Sistem navigate ke Tab "Daily Monitoring" dan load interface. |
| 002 | tampil-tab-daily-monitoring-terdapat-data-disana.png | **Daily Monitoring Dashboard - Patient Monitoring Table:** **Header Section:** Title, Subtitle menjelaskan purpose monitoring. **Filter & Control Section:** Date Range picker, Risk Level dropdown (options: Normal/Caution/Alert/Excess), Status dropdown (options: Active/About to Discharge), Search Box (patient name/MR), Manual Refresh button, Auto-refresh info indicator. **Patient Table Columns:** Checkbox, No., Patient Name (clickable), MR Number, Room/Kamar, Admission Date, Days Admitted, Benefit Available (Rp format), Benefit Used (Rp format), Benefit Remaining (Rp format), Usage % (visual progress bar with color coding: Green/Yellow/Orange/Red based on range), Risk Indicator (icon based on risk level), Action Buttons (View Detail, Add Data). Pagination display. |
| 003a | tampil-form-tambah-data-daily-monitoring.png | **Quick Add Monitoring Data Form:** Modal dialog "Tambah Data Monitoring - Input Cepat" terbuka saat user klik Add Data button. **Quick Form Fields:** Select Patient (required, dropdown with search/autocomplete), Service Category (required, dropdown options), Amount (required, numeric Rp format), Date of Service (required, date picker), Notes (optional, textarea). **Action Buttons:** Save button, Cancel button, Expand Form link. **Validation on Submit:** Required field validation dengan inline error messages dan field highlighting. Simple quick entry form. |
| 003b | tampilan-form-tambah-daily-monitoring-complete.png | **Detailed Monitoring Data Form:** Modal dialog "Tambah Data Monitoring - Form Lengkap" menampilkan expanded form dengan multiple sections. **Section 1 - Patient Info (read-only):** Nama Pasien, Nomor Rekam Medis, Kamar, Tanggal Admission, Dokter Pengasuh. **Section 2 - Service Details:** Service Code (required, dropdown with search), Service Description (auto-fill, read-only), Service Category (auto-fill, read-only), Service Date (required, date picker), Quantity (required, numeric), Unit Price (read-only, Rp format), Total Amount (auto-calculated read-only). **Section 3 - Benefit Calculation (auto-calculated, real-time):** Mapped to Benefit Category (auto-suggest, user editable), Coverage % (read-only), Estimated Benefit Amount (auto-calculated real-time), Co-payment Amount (auto-calculated). **Section 4 - Notes:** Textarea optional. **Real-time Calculation Display:** Shows Service Amount, Coverage %, Benefit Amount, Co-payment updating in real-time. **Action Buttons:** Save button, Cancel button, Collapse to Quick Form link. Full detail entry form untuk precision input dengan auto-calculation. |
| 004 | daily-monitoring-berhasil-ditambahkan.png | **Confirmation Screen - Data Monitoring Disimpan:** Green success message card ✓ "Data Berhasil Disimpan" dengan subtitle dan timestamp. **Summary Data Entered:** Service Details (Service name, Category, Amount, Quantity, Date) display, Benefit Calculation (Coverage %, Benefit Amount, Co-payment) display. **Updated Patient Status:** Patient info display. **Before & After Comparison:** Before Entry section showing previous Benefit Used, Benefit Remaining, Usage %, After Entry section showing updated values, visual indicator menunjukkan perubahan status. **Alert Notification (if triggered):** Alert message display (jika usage > threshold), notification delivery info. **Action Buttons:** Kembali ke Dashboard button (auto-refresh), Tambah Data Lagi button (open form again), Lihat Detail Pasien button (view full patient details). Success feedback dengan impact visualization. |
